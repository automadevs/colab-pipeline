...
- `scripts/master_pipeline.py`: faz o setup do repositório (`git pull --ff-only` ou clone `--depth 1`), inspeciona o ambiente (Python, Kaggle CLI, Civitai CLI, huggingface_hub, secrets), coleta e resolve todos os inputs (com todas as perguntas interativas antecipadas), exibe o **INPUT RESOLUTION SUMMARY** e, se alguma entrada falhar, aborta com erro **antes** de qualquer edição do dataset ou download; caso contrário, executa a fila de downloads sem pausas e **só publica se todos os downloads do lote tiverem sucesso** (falha de download → `[DOWNLOAD FAILED]`, dataset remoto inalterado).
- `collect_input_queue()` + `resolve_queue_metadata()` + `download_resolved_queue()` (em `kaggle_dataset_manager.py`): coletam todos os AIRs/URLs/HF primeiro (validando cada entrada via `parse_input`, que delega para `validate_air`/`validate_civitai_url`/parser HF, sem interromper o loop em caso de erro), resolvem os metadados de todo o lote em uma `ResolutionOutcome` (artefatos + falhas; o destino de checkpoints é perguntado uma única vez por lote e a classificação pendente é resolvida antes do resumo) e só depois baixam sequencialmente — um arquivo por vez, com retry automático (`@retry`, 3 tentativas, backoff 2s→4s) em erros transitórios de rede. **Falha de resolução em qualquer item aborta o pipeline (sem dataset parcial); falha permanente de download em qualquer item também aborta a publicação do lote** (`DownloadOutcome` com falhas → `[DOWNLOAD FAILED]`, staging limpo conforme a política de cache HF, dataset remoto inalterado).
